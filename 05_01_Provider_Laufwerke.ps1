...
#Multiple comma-separated paths are converted to absolute paths by the Resole-Path cmdlet and output
Resolve-Path -Path ~, /, ..
...
#The D drive of the Filesystem provider is created. Root directory is the specified network share
...
(Get-Item .\schutz.xlsx).IsReadOnly
...
#The value of the environment variable USRENAME is set to the value Nicholas
...
#Text is added here
New-Item -Path . -Name "test.txt" -ItemType "file" -Value "Erster Eintrag..."

#For lines create a folder (do not forget -ItemType with the Value Directory)
...
Set-ItemProperty -Path .\fertig.docx -Name IsReadOnly -Value $True

Get-ItemProperty -Path .\fertig.docx
...
#One last example
Remove-ItemProperty -Path HKLM:\SOFTWARE\ZumTesten
